...
# --- KONFIGURACJA KOYEB (Fix dla pętli zdarzeń) ---
import telegram.ext
class DummyJobQueue:
...
async def text_to_speech(text):
    """Zamienia tekst na głos AI z wykładniczym backoffem."""
...
async def handle_gpt(update: Update, text_command: str, image_b64: str = None):
    ...
    sys_prompt = (
        ...
        "używasz 'kurwa', ale pod żadnym pozorem nie obrażaj rozmówcy i nie nazywaj go debilem. "
        ...
        f"{static_data}\n\n"
        "ZASADA: Jesteś modelem AI, więc znasz się na wszystkim. Jeśli pytanie dotyczy gry, "
        "używaj logów. Jeśli świata, używaj swojej wiedzy ogólnej. Jeśli czegoś nie wiesz, powiedz 'nie wiem kurwa'."
    )

    url = f"https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key={API_KEY}"
    parts = [{"text": query if query else "Analizuj ten obraz."}]
    if image_b64:
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            # Generuj i wyślij nagranie głosowe
            voice_data = await text_to_speech(answer)
            if voice_data:
                await update.message.reply_voice(voice=io.BytesIO(voice_data))
        else:
            await update.message.reply_text("Kurwa, błąd API czatu.")
    except Exception as e:
        print(f"GPT Error: {e}")
...
async def handle_img(update: Update, text: str):
    ...
    wait_msg = await update.message.reply_text("Rysuję to, czekaj chwilę...")
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            await wait_msg.edit_text("Kurwa, błąd generatora obrazów.")
    except Exception as e:
        print(f"Img Error: {e}")
        await wait_msg.edit_text("Coś się zjebało przy rysowaniu.")
...
async def on_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ZAPISYWANIE DO PAMIĘCI BIEŻĄCEJ (jeśli to nie komenda)
